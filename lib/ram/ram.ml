...
module RAM = struct
    ...
    let byte_to_uint8 (byte : char) =
        Char.code byte |> UInt8.from_int

    let uint8_to_byte (u8 : uint8) =
        UInt8.to_int u8 |> Char.chr
    ...
    let read_ui16 (ram : t) (addr : uint16) : uint16 =
        ...
        let least_sig_bit = Bytes.get ram.ram_memory mem_addr_int |> byte_to_uint8 in
        let most_sig_bit = Bytes.get ram.ram_memory (mem_addr_int + 1) |> byte_to_uint8 in
