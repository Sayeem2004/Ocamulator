...
module F (M : S) = struct
    ...
    let (logxor_tests : test list) =
        [
            ...
            logxor_test "Logxor Ones Maxx -> Maxx - 1" M.one M.max_value
                (M.pred M.max_value);
    ...
    let (shift_left_tests : test list) =
        [
            ...
            shift_left_test "Shift_left Ones Size -> Over" M.one M.size
                (M.from_int over);
            shift_left_test "Shift_left Maxx 1 -> Maxx - 1" M.max_value 1
                (M.pred M.max_value);
        ]
    ...
    let (shift_right_tests : test list) =
        [
            ...
            shift_right_test "Shift_right Maxx 1 -> Maxx / 2" M.max_value 1
                (M.from_int ((over - 1) / 2));
            shift_right_test "Shift_right Maxx Size - 1 -> Ones" M.max_value
                (M.size - 1) M.one;
        ]
